...
import type { FundingResultsPayload } from '@/lib/ai/stream'

type ToolResult = { result: string; funding?: FundingResultsPayload }
...
/** Async tool: runs the deterministic DTx Funding Finder and returns both a model
 * summary and the full structured results (for the UI cards). */
async function findDtxFunding(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!region) {
    return { result: JSON.stringify({ error: 'Missing region — ask the user which NHS England region first.' }) }
  }
  if (!app && !condition) {
    return {
      result: JSON.stringify({
        error: 'Missing condition or app — ask the user which condition or DTx app to search funding for.',
      }),
    }
  }

  const mode: SearchMode = app ? SEARCH_MODE.APP : SEARCH_MODE.CONDITION
  const selection = (app ?? condition) as string

  try {
    const results = await runFundingFinder({ region, mode, selection })
    const selectionLabel = getSelectionLabel(mode, selection)

    const summary = results.map((r: FundingResult) => ({
      fund_name: r.fund_name,
      provider: r.provider,
      score: r._score,
      match_type: r.match_type,
      applicant_fit: r.applicant_fit,
    ...
    return {
      result: JSON.stringify({
        ...
        results: summary,
        render_instruction:
          'The full result cards are already shown to the user. Give a brief conversational framing and 1-2 next steps — do not re-list every fund or its details.',
      }),
      funding: { results, region, selectionLabel },
    }
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Funding search failed'
    return { result: JSON.stringify({ error: message }) }
  }
}

const asyncExecutors: Record<
  string,
  (args: Record<string, unknown>) => Promise<ToolResult>
> = {
  find_dtx_funding: findDtxFunding,
}
...
export async function executeTool(
  ...
  const asyncExecutor = asyncExecutors[name]
  if (asyncExecutor) {
