...
export function getCommissionerProfileFromSession(session: SessionLike): CommissionerProfile {
  ...
  if (session.profileDisplayName || session.profileOrganisationName) {
    return {
      ...
      ruralUrbanMix: '',
      starterPrompts: [
        { label: 'Explore COPD apps', prompt: 'What COPD digital health apps are available in the catalogue and how do they compare?' },
        { label: 'Digital cardiac rehab', prompt: 'What digital cardiac rehabilitation options are available and what does the evidence say?' },
        { label: 'Available funding', prompt: 'What funding opportunities are available for digital health commissioning?' },
        { label: 'Build a business case', prompt: 'Help me structure a business case for digital long-term condition management for our board.' },
      ],
    }
  }
