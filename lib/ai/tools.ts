...
export const aiTools = [
  ...
  {
    type: 'function' as const,
    name: 'find_dtx_funding',
    description:
      'Run the NHS DTx Funding Finder: searches current NHS funding, adoption, and commissioning routes for deploying a digital therapeutic, scores each 0-100 deterministically, and filters to opportunities a commissioning/providing NHS organisation can actually access. ' +
      'ONLY call this once you know (a) the NHS England region and (b) EITHER a condition OR a specific DTx app. If either is missing or ambiguous, ask the user first (offer clickable suggestions). Always assume the applicant is a commissioner. Do not invent funding — this tool returns the real results, which are shown to the user as rich cards automatically.',
    parameters: {
      type: 'object',
      properties: {
        region: {
          type: 'string',
          description: 'The NHS England region the commissioner is in.',
          enum: [
        ...
        condition: {
          type: 'string',
          description:
            'Search by condition. Use this OR app, not both. copd = COPD, pr = Pulmonary Rehabilitation, cr = Cardiac Rehabilitation.',
          enum: ['copd', 'pr', 'cr'],
        },
        app: {
          type: 'string',
          description:
            'Search by a specific DTx app. Use this OR condition, not both.',
          enum: [
            'mycopd',
            'clinitouch',
            'copdhub',
